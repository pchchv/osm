...
func TestCompute_MissingChildren(t *testing.T) {
	...
	if _, err := Compute(ctx, parents, ds, opts); err != nil {
		t.Fatalf("compute error: %e", err)
	}
...
func TestCompute_DeletedParent(t *testing.T) {
	...
	// last living parent needs to have updates up to the deleted time
...
func TestCompute_ChildUpdateAfterLastParentVersion(t *testing.T) {
	ctx := context.Background()
	// if a child is updated after the only version of a parent, an update should be created
	ds := &TestDS{}
	ds.Set(child1, ChildList{
		&shared.Child{ID: child1, Version: 0, VersionIndex: 0, Timestamp: start.Add(0 * time.Hour), Visible: true},
		&shared.Child{ID: child1, Version: 1, VersionIndex: 1, Timestamp: start.Add(1 * time.Hour), Visible: true},
...
func TestCompute_ChildUpdateRightBeforeParentDelete(t *testing.T) {
	ctx := context.Background()
	// if a child is updated right before (based on threshold) a parent is DELETED, this should create an update
	// this also tests a child is missing in the next version
	// all of these histories should returned the same results
	dss := []*TestDS{}
	ds := &TestDS{}
	ds.Set(child1, ChildList{
...
func TestCompute_ChildUpdateRightBeforeParentUpdated(t *testing.T) {
	ctx := context.Background()
	// if a child is updated right before (based on threshold) a parent is UPDATED, this should not trigger an updates
	ds := &TestDS{}
	ds.Set(child1, ChildList{
		&shared.Child{ID: child1, Version: 0, VersionIndex: 0, Timestamp: start.Add(0 * time.Hour), Visible: true},
...
func TestCompute_MultipleChildren(t *testing.T) {
	ctx := context.Background()
	// parent with multiple children should handle each child independently
	ds := &TestDS{}
	ds.Set(child1, ChildList{
		&shared.Child{ID: child1, Version: 0, VersionIndex: 0, Timestamp: start.Add(0 * time.Hour), Visible: true},
		&shared.Child{ID: child1, Version: 1, VersionIndex: 1, Timestamp: start.Add(1 * time.Hour), Visible: true},
...
func TestCompute_ChangedChildList(t *testing.T) {
	ctx := context.Background()
	// change in the child list should be supported
	ds := &TestDS{}
	ds.Set(child1, ChildList{
		&shared.Child{ID: child1, Version: 0, VersionIndex: 0, Timestamp: start.Add(0 * time.Hour), Visible: true},
		&shared.Child{ID: child1, Version: 1, VersionIndex: 1, Timestamp: start.Add(1 * time.Hour), Visible: true},
...
func TestCompute_MajorChildren(t *testing.T) {
	...
	if _, err := Compute(ctx, parents, ds, &Options{Threshold: time.Minute}); err != nil {
		t.Fatalf("setup error: %e", err)
	}
	...
	if _, ok := err.(*NoVisibleChildError); !ok {
		t.Errorf("did not return correct error, got %e", err)
	}
