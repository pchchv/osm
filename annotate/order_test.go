...
func TestChildFirstOrdering(t *testing.T) {
	...
	ordering := NewChildFirstOrdering(context.Background(), relations.IDs(), (&osm.OSM{Relations: relations}).HistoryDatasource())
	...
	if err := ordering.Err(); err != nil {
		t.Errorf("unexpected error, got %e", err)
	}
}
...
func TestChildFirstOrdering_Cancel(t *testing.T) {
	...
	ordering := NewChildFirstOrdering(ctx, relations.IDs(), (&osm.OSM{Relations: relations}).HistoryDatasource())
	ordering.Next()
	ordering.Next()
	done()
	...
	if err := ordering.Err(); err != context.Canceled {
		t.Errorf("incorrect error, got %e", err)
	}
}
...
func TestChildFirstOrdering_Walk(t *testing.T) {
	...
	for _, r := range relations {
		if err := ordering.walk(r.ID, path); err != nil {
			t.Errorf("should process cycle without problem: %e", err)
		}
	}
}
