...
func TestScanner(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("unable to open file: %e", err)
	}
	defer f.Close()
...
func TestScanner_Header(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("error reading header: %e", err)
	}
...
func TestScanner_FullyScannedBytes(t *testing.T) {
	...
	t.Run("should always be increasing after restart", func(t *testing.T) {
		...
		if _, err = f.Seek(214162, 0); err != nil {
...
func TestScanner_FilterNodes(t *testing.T) {
	var nodes, ways, relations, nodesFiltered, waysFiltered, relationsFiltered int
	f, err := os.Open(Delaware)
	if err != nil {
		t.Fatalf("unable to open file: %e", err)
	}
	...
	if err := scanner.Close(); err != nil {
		t.Errorf("scanneer close returned error %e", err)
	}

	// read file again
	if _, err := f.Seek(0, 0); err != nil {
		t.Errorf("seek to begin of file failed %e", err)
	}
