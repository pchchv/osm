...
// Join joins a set of segments into a set of connected MultiSegments.
func Join(segments []Segment) []MultiSegment {
	lists := []MultiSegment{}
	segments = compact(segments)
	// matches are removed from `segments` and
	// put into the current group,
	// so when `segments` is empty its done
	for len(segments) != 0 {
		current := MultiSegment{segments[len(segments)-1]}
		segments = segments[:len(segments)-1]
		// if the current group is a ring, its done
